 ...
 */
        private static $ops = array(
            ...
            'addtheme'      => array(TRUE, TRUE, FALSE),
 ...
 * @return void
 */
        private function addtheme($context)
        {
            $fdt = $context->formdata();
            $theme = R::dispense('theme');
            $theme->name = $fdt->mustpost('themename');
            $theme->description = $fdt->mustpost('description');
            R::store($theme);
            echo $theme->getID();
        }
 ...
 * @return void
 */
        private function addproject($context)
        {
            $fdt = $context->formdata();
            $prjct = R::dispense('project');
            $prjct->name = $fdt->mustpost('topicname');
            $prjct->description = $fdt->mustpost('description');
//            $prjct->supervisor =
            R::store($prjct);
            echo $prjct->getID();
        }
 ...
 * @return void
 */
        private function edittheme($context)
        {
            $fdt = $context->formdata();
            $theme = R::dispense('theme');
            $theme->name = $fdt->mustpost('themename');
 ...
 */
        private function adduser($context)
        {
            ...
            if ($fdt->post('supervisor', 0) == 1)
            {
                $u->addrole('Site', 'Supervisor', '', $now);
            }
            if ($fdt->post('themeleader', 0) == 1)
            {
                $u->addrole('Site', 'Themeleader', '', $now);
            }
            if ($fdt->post('moduleleader', 0) == 1)
            {
                $u->addrole('Site', 'Moduleleader', '', $now);
            }
            echo $u->getID();
        }
/**
